...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  # layer 1
  # Source: https://stackoverflow.com/questions/46983334/adding-two-addtional-layers-to-inception-with-transfer-learning?rq=1
  layer_name = 'layer_1_relu'
  with tf.name_scope(layer_name):
    ...
    with tf.name_scope('relu_activation'):
      ...
      tf.summary.histogram('final_relu_activations', logits)

  # layer 2
  # Source: https://stackoverflow.com/questions/46983334/adding-two-addtional-layers-to-inception-with-transfer-learning?rq=1
  layer_name = 'layer_2_tanh'
  with tf.name_scope(layer_name):
    with tf.name_scope('weights'):
      initial_value = tf.truncated_normal(
          [1024, 1024], stddev=0.001)
      layer_weights = tf.Variable(initial_value, name='final_weights')
      variable_summaries(layer_weights)
    ...
    with tf.name_scope('tanh_activated'):
      tanh_activated = tf.nn.tanh(logits, name="tanh")
      tf.summary.histogram('final_tanh_activations', logits)
  ...
  with tf.name_scope(layer_name):
    ...
    with tf.name_scope('Wx_plus_b'):
      logits = tf.matmul(tanh_activated, layer_weights) + layer_biases
      tf.summary.histogram('pre_activations', logits)
...
#python retrain_with_device.py --image_dir flower_photos_light


#python retrain_device_relu_sigmoid.py --image_dir food41/train/ --tfhub_module https://tfhub.dev/google/imagenet/mobilenet_v1_100_224/quantops/feature_vector/1
